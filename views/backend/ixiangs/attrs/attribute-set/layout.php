...
$f->newField('')->setRenderer(function($field) use($langId){
    ...
    $res[] = '<div class="panel-heading">'.$this->locale->_('attrs_assigned_group').'</div>';
    ...
    $res[] = '<div class="panel-heading">'.$this->locale->_('attrs_assignable_group').'</div>';
...
<?php $this->nextBlock('footerjs'); ?>
    <script language="javascript">
        ...
        $('#save').click(function(){
            ...
            if(!passed){
                alert('<?php echo $this->locale->_('attrs_err_set_not_empty'); ?>');
            }else{
                $f.submit();
            }
        });
