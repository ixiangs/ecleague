...
$dt->addOptionColumn($this->locale->_('status'), '{status}', array(
        \Ixiangs\User\AccountModel::STATUS_ACTIVATED=>'<span class="text-success">'.$this->locale->_('auth_status_activated').'</span>',
        \Ixiangs\User\AccountModel::STATUS_NONACTIVATED=>'<span class="text-warning">'.$this->locale->_('auth_status_nonactivated').'</span>',
        \Ixiangs\User\AccountModel::STATUS_DISABLED=>'<span class="text-danger">'.$this->locale->_('disabled').'</span>'),
    'small', 'small text-center');
