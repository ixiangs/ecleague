...
<style>
    ...
    td{padding: 5px;}
    .red td {background-color: #ff7777;}
    td.line, td.no{text-align: center;}
    td.method, td.classname, td.file, td.message{}
</style>
</head>
<body>
    <p>测试完成，共<?php echo count($this->_testSuccess) + count($this->_testFailures)?> 成功 <?php echo count($this->_testSuccess);?> 失败 <?php echo count($this->_testFailures)?></p>
