...
$this->assign('breadcrumb', array(
    ...
    $this->html->anchor($this->locale->_($this->router->action == 'add'?'entities_new_group':'entities_edit_group'))
));
...
if($this->model->id):
    $f->addStaticField($this->locale->_('entities_owner_component'), $this->components[$this->model->getComponentId()]);
else:
    $f->newField($this->locale->_('entities_owner_component'), true,
...
<!---->
<!--            if(!passed){-->
<!--                alert('--><?php //echo $this->locale->_('entities_err_group_not_empty'); ?><!--');-->
<!--            }else{-->
<!--                $f.submit();-->
<!--            }-->
<!--        });-->
