...
$this->assign('navigationBar', array(
    $this->html->anchor($this->localize->_('back'), $this->router->findHistory('list', array('languageid'=>$this->language->getId()))),
));

$this->assign('toolbar', array(
    $this->html->button('button', $this->localize->_('add'), 'btn btn-success')->setAttribute('id', 'add'),
...
?>
    <script language="javascript">
        ...
        var fieldHtml = '<div class="form-group">' +
            '<label class="control-label" for="codes_{index}"><?php echo $this->localize->_('code'); ?></label>' +
            '<input type="text" value="" data-validate-required="true" id="codes_{index}" name="codes[{index}]" class="form-control">' +
            '</div>' +
            '<div class="form-group">' +
            '<label class="control-label" for="labels_{index}"><?php echo $this->localize->_('text'); ?></label>' +
            '<input type="text" value="" data-validate-required="true" id="labels_{index}" name="labels[{index}]" class="form-control">' +
            '</div>'+
            '<div class="form-group">' +
            '<button type="button" class="btn btn-default" id="delete_{index}" onclick="javascript:deleteField({index});"><?php echo $this->localize->_('delete'); ?></button>' +
            '</div>';
