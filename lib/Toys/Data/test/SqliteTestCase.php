...
class SqliteTestCase extends Toys\Unit\TestCase{

    private $_db = null;

    public function __construct(){
        $this->_db = new \Toys\Data\Db\SqliteProvider(array('dsn' => 'sqlite:'.dirname(__FILE__).DS.'testdb.db3'));
        $this->_db->connect();
    }

    public function testInsert(){
        $is = new \Toys\Data\Sql\InsertStatement('people', array(
            'fullname'=>'ronald',
            'age'=>20,
            'gender'=>1,
            'address'=>'china'
    ...
    public function testUpdate(){
        ...
        $us = new \Toys\Data\Sql\UpdateStatement('people', array(
            'fullname'=>'ronald'.$lid,
            'age'=>20 + $lid
        ));
        $us->eq('id', $lid);
    ...
    public function testDelete(){
        ...
        $lid = $this->_db->getLastInsertId();
        $us = new \Toys\Data\Sql\DeleteStatement('people');
        $us->eq('id', $lid);
        $this->_db->delete($us);
    }

    public function testSelect(){
        $ss = new \Toys\Data\Sql\SelectStatement('people');
        $rs1 = $this->_db->select($ss);
        $rs2 = $this->_db->fetch('SELECT * FROM people');
        $this->assertEqual(count($rs1->rows), count($rs2->rows));

        $ss = new \Toys\Data\Sql\SelectStatement('people', array('address'));
        $rs1 = $this->_db->select($ss);
        ...
        $ss = new \Toys\Data\Sql\SelectStatement('car');
